...
function Queue(process, opts) {
  ...
  self.cancelIfRunning =
    opts.cancelIfRunning === undefined ? false : !!opts.cancelIfRunning;
  self.autoResume = opts.autoResume === undefined ? true : !!opts.autoResume;
  self.failTaskOnProcessException =
    opts.failTaskOnProcessException === undefined
      ? true
      : !!opts.failTaskOnProcessException;
...
Queue.prototype.getStats = function () {
  var successRate =
    this._processedTotal === 0
      ? 0
      : 1 - this._failedTotal / this._processedTotal;
  return {
...
Queue.prototype.use = function (store, opts) {
  ...
  var loadStore = function (store) {
    ...
    try {
      ...
      throw new Error(
        "Attempting to require better-queue-" +
          store +
          ", but failed.\nPlease ensure you have this store installed via npm install --save better-queue-" +
          store
      );
    }
    return Store;
  ...
  if (typeof store === "string") {
    ...
    store.putTask &&
    store.getTask &&
    ((self.filo && store.takeLastN) || (!self.filo && store.takeFirstN))
  ) {
    self._store = store;
  } else {
  ...
  self._store.connect(function (err, len) {
    ...
    self._store.getRunningTasks(function (err, running) {
      if (!self._stopped && self.autoResume) {
        const lockIds = Object.keys(running);
      ...
      for (var i = 0; i < self._tasksWaitingForConnect.length; i++) {
        self.push(
          self._tasksWaitingForConnect[i].input,
          self._tasksWaitingForConnect[i].ticket
        );
      }
    });
  });
...
Queue.prototype.resume = function () {
  ...
  const workers = self._getWorkers();
...
Queue.prototype._getWorkers = function () {
  ...
  const selfWorkers = Object.keys(self._workers);
  for (let i = 0; i < selfWorkers.length; i++) {
    const taskId = selfWorkers[i];
    var worker = self._workers[taskId];
...
Queue.prototype._queueTask = function (taskId, newTask, ticket) {
  ...
  var putTask = function () {
    ...
    // Save ticket
    var tickets =
      (self._writing[taskId] && self._writing[taskId].tickets) || new Tickets();
...
Queue.prototype._startBatch = function (batch, tickets, lockId) {
  ...
  var updateStatsForEndedTask = function (taskId) {
    ...
    if (elapsed > 0) {
      ...
      self._processedAverage =
        self._processedTotalElapsed / self._processedTotal;
    }
    return stats;
  };
