...
Worker.prototype.setup = function () {
  ...
  // Setup
  for (let id = 0; id < self._taskIds.length; id++) {
...
Worker.prototype.failedBatch = function (msg) {
  ...
  const waitingKeys = Object.keys(self._waiting);
  for (let i = 0; i < waitingKeys.length; i++) {
    const id = waitingKeys[i];
    if (!self._waiting[id]) continue;
...
Worker.prototype.progressBatch = function (complete, total, msg) {
  ...
  for (let i = 0; i < self._taskIds.length; i++) {
    self.progress.complete += self.progress.tasks[i].pct;
  }
